...
var (
	...
	// 宝塔/防火墙安全检测页会通过 JS 跳转到带 btwaf 参数的真实页面
	securityRedirectRegex = regexp.MustCompile(`window\.location\.href\s*=\s*["']([^"']*btwaf=[^"']+)["']`)
...
func (p *DjgouPlugin) searchImpl(client *http.Client, keyword string, ext map[string]interface{}) ([]model.SearchResult, error) {
	...
	// 兼容新版 Z-Blog 列表结构和旧版 erx 列表结构
	currentItems := doc.Find("article.post-item-row")
	if currentItems.Length() > 0 {
		currentItems.Each(func(i int, s *goquery.Selection) {
			result := p.parseCurrentSearchItem(s)
			if result.UniqueID != "" {
		...
		if mainListSection.Length() == 0 {
			return nil, fmt.Errorf("[%s] 未找到搜索结果容器", p.Name())
		}
...
func (p *DjgouPlugin) parseCurrentSearchItem(s *goquery.Selection) model.SearchResult {
	...
	result.Title = title
	result.Datetime = p.parseTime(strings.TrimSpace(s.Find(".post-date").First().Text()))
	result.Tags = []string{strings.TrimSpace(s.Find(".post-cate-badge a").First().Text())}
	result.Channel = ""
	result.Content = link
...
func (p *DjgouPlugin) parseTime(timeStr string) time.Time {
	...
	if t, err := time.Parse("01-02", timeStr); err == nil {
		now := time.Now()
		return time.Date(now.Year(), t.Month(), t.Day(), 0, 0, 0, 0, now.Location())
	}
...
func (p *DjgouPlugin) fetchDetailPage(client *http.Client, detailURL string) ([]model.Link, string) {
	...
	// 查找主内容区域（用于提取简介）
	mainContent := doc.Find("article.post-detail")
	if mainContent.Length() == 0 {
		mainContent = doc.Find("div.erx-wrap")
	}
	if mainContent.Length() == 0 {
		mainContent = doc.Find("main.main-content")
	}
	if mainContent.Length() == 0 {
		return nil, ""
	}
...
func (p *DjgouPlugin) doRequestWithRetry(req *http.Request, client *http.Client) (*http.Response, error) {
	...
	for i := 0; i < maxRetries; i++ {
		...
		if err == nil && resp.StatusCode == 200 {
			resp, err = p.followSecurityRedirect(resp, reqClone, client)
			if err == nil && resp.StatusCode == 200 {
				return resp, nil
			}
		}
...
func (p *DjgouPlugin) followSecurityRedirect(resp *http.Response, req *http.Request, client *http.Client) (*http.Response, error) {
	...
	match := securityRedirectRegex.FindSubmatch(body)
	if len(match) < 2 {
	...
	nextURL, err := req.URL.Parse(string(match[1]))
	if err != nil {
		return nil, err
	}
	...
	nextReq.Header.Set("Referer", SiteURL)

	return client.Do(nextReq)
}
