...
- [资源库与自动采集 V1](docs/resource-library-v1.md)：PostgreSQL、管理台、自动采集、备份与上线检查。

## 资源库与自动采集（可选）

配置 `DATABASE_URL` 后，PanSou 会在启动时自动执行 PostgreSQL migration，并启用数据库优先的混合搜索、关键词采集调度和 `/admin/` 管理台。再设置 `AUTH_ENABLED=true`、`ADMIN_USERNAME`、`ADMIN_PASSWORD` 与持久化的 `AUTH_JWT_SECRET`，即可启用 PostgreSQL 多用户门户、角色权限、个人 API Key、按用户限流和 30 天调用监控。未配置数据库时继续使用原有实时搜索与磁盘缓存，不改变现有部署行为。
...
管理员登录 `/admin/` 后，在“搜索来源”中启停 TG 频道和内置插件，点击“保存并生效”即可热更新，无需重启服务。四个账号型插件启用后，管理员可添加管理员私有或公开共享账号；普通用户在门户“搜索账号”中维护自己的独立多账号配置。用户搜索优先使用个人账号，个人账号整层失败后才回退公开共享账号。旧版匿名插件管理路由在数据库模式下不再开放。

主密钥可用 `openssl rand -base64 32` 生成，必须与 PostgreSQL 一起备份。系统会把旧 `CACHE_PATH/{qqpd,gying,panlian,weibo}_users` 账号一次性加密迁移为管理员私有账号，并保留原文件不动。
...
- Go 1.24.1+（建议使用仓库声明的 `go1.24.9` toolchain）
...
| **AUTH_USERS** | 旧版用户账号配置 | 无 | 数据库用户表为空且未设置 `ADMIN_*` 时，仅将首个账号作为兼容引导管理员 |
| **AUTH_TOKEN_EXPIRY** | Token有效期（小时） | `24` | JWT Token的有效时长 |
| **AUTH_JWT_SECRET** | JWT签名密钥 | 自动生成 | 数据库多用户模式必须显式设置并持久化 |
...
| SEARCH_RESPONSE_TIMEOUT_SECONDS | 搜索接口前台软响应预算(秒)，超时后返回处理中并继续后台搜索 | `10` |
